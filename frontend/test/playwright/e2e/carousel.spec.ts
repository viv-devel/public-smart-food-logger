...
test.describe('HowItWorks Carousel', () => {
  ...
  test('should display the carousel and allow navigation', async ({ page }) => {
    ...
    const step1Heading = page.getByRole('heading', { name: 'AIに食事を伝える' });
    await expect(step1Heading).toBeVisible({ timeout: 10000 });
    // Text is split by <strong> tags, so we check for parts
    await expect(page.getByText('写真を渡すだけでなく、「今日のお昼はペペロンチーノとサラダ」のように')).toBeVisible();
    ...
    await expect(page.getByText('カスタムGeminiが生成した栄養情報（JSON）をフォームに貼り付け、')).toBeVisible();
    ...
    await expect(page.getByText('記録された食事はFitbitアプリで')).toBeVisible();
